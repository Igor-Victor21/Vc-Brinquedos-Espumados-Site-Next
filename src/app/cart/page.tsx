...
export default function CartPage() {
  ...
  const total = produtos.reduce((acc, produto) => {
    ...
    const precoFinal = itemCarrinho?.finalPrice ?? produto.price
    if (!itemCarrinho) return acc

    const quantidade = itemCarrinho.quantity
    const valor = itemCarrinho.finalPrice ?? produto.price // usa o preço com desconto se existir
  ...
  const handleZap = () => {
    ...
    const itemList = produtos.map((produto) => {
      ...
      const itemCarrinho = cartItems.find(item => item.id === produto.id)
      const valorUnitario = itemCarrinho?.finalPrice ?? produto.price
